...
function Dropdown({ name, items, onClick }: DropdownProps) {
	const [isActiveDropdown, setIsActiveDropdown] = useState<boolean>(false);
	const [races, setRaces] = useState([]);
	//const [additionalDataArray, setAdditionalDataArray] = useState([]);
	const [isLoading, setIsLoading] = useState<boolean>(true);
	const handleToggleDropdown = () => {
		setIsActiveDropdown(!isActiveDropdown);
	};

	// const fetchAdditionalDataForRace = async (title) => {
	// 	try {
	...
	// 	} catch (error) {
	// 		console.error(
	// 			`Помилка при отриманні додаткових даних для ${title}`,
	// 			error
	// 		);
	// 		return null; // Або ви можете обробити помилку якось і повернути відповідний результат
	// 	}
	// };

	const fetchRaces = async () => {
		try {
			// Встановлюємо стан loading на true перед початком завантаження
			setIsLoading(true);

			const response = await apiClient.get(
				`${import.meta.env.VITE_API_URL}2023.json`
			);
			const { data } = response;
			...
			// Навіть якщо виникла помилка, або завантаження було успішним,
			// встановлюємо стан loading на false після завершення операції
			setIsLoading(false);
		}
	};
	// useEffect(() => {
	...
	// 				return await fetchAdditionalDataForRace(
	// 					race.Circuit.circuitName
	// 				);
	// 			});

	// 			const additionalDataArray = await Promise.all(fetchPromises);

	// 			setAdditionalDataArray(additionalDataArray);
	// 		} catch (error) {
	...
	useEffect(() => {
		fetchRaces();
	}, []); // Тут використовуємо пустий масив для відтворення componentDidMount
	...
	return (
		<div className='w-fit relative'>
			<div
				...
				className='flex text-textColor items-center cursor-pointer w-72 text-xl border-[1px] border-textColor px-2 py-2 justify-between rounded-md'>
				...
				<div className='absolute w-full top-14 right-0 flex overflow-y-auto h-72 rounded-md border-[1px] border-textColor'>
					<div className='h-fit w-full flex flex-col'>
						{races.map((item, index) => (
							<div
								key={index}
								className='h-24 cursor-pointer w-full border-b-[1px] border-textColor flex justify-between items-center p-2 !text-xs text-textColor flex-col'>
								<div className='h-5/6 w-full flex justify-between'>
									<div className='flex flex-col justify-evenly'>
										...
										<InfoText
											label='Country'
											text={item.Circuit.Location.country}
										/>
										<InfoText
											label='Date'
											text={item.Circuit.Location.country}
